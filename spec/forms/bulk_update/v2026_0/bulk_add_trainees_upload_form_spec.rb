...
describe BulkUpdate::BulkAddTraineesUploadForm, type: :model do
  ...
  context "when passed file with just a header row" do
    let(:test_file_contents) { BulkUpdate::AddTrainees::V20260::ImportRows::ALL_HEADERS.keys.join(",") }
  ...
  context "when passed a valid file" do
    let(:valid_columns) { BulkUpdate::AddTrainees::V20260::ImportRows::ALL_HEADERS.keys.join(",") }
    ...
    it "returns no validation errors and creates a BulkUpdate::TraineeUpload record" do
      ...
      expect(upload.version).to eq(version)
  ...
  context "when passed a file with mixed case headings" do
    let(:mixed_case_headings) { BulkUpdate::AddTrainees::V20260::ImportRows::ALL_HEADERS.keys.map.with_index { |header, index| index.odd? ? header.upcase : header.downcase } }
