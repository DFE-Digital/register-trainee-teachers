...
RSpec.describe Api::V20250::TraineeAttributes do
  ...
  describe "validations" do
    ...
    describe "hesa_id" do
      describe "length" do
        subject { described_class.new(hesa_id: SecureRandom.random_number(10**character_length).to_s.rjust(character_length, "0")) }

        context "with 13 chars" do
          let(:character_length) { 13 }
    ...
    describe "study_mode" do
      ...
      context "when not included in the list of HESA study modes" do
        ...
        it {
          ...
          expect(subject.errors[:study_mode]&.first).to match(/has invalid reference data value of '.*'. Valid values are #{ReferenceData::STUDY_MODES.hesa_codes.map { |v| "'#{v}'" }.join(', ')}/)
        }
      end

      context "when included in the list of HESA study modes" do
        ReferenceData::STUDY_MODES.names.each do |study_mode|
          subject { described_class.new(study_mode:) }
      ...
      describe "inclusion" do
        context "when trainee_start_date is present" do
          ...
          context "when AcademicCycle::start_date == 2023" do
            ...
            it "includes full details in the error message" do
              ...
              expect(subject.errors[:training_route]).to include(
                "has invalid reference data value of '9'. Valid values are #{ReferenceData::TRAINING_ROUTES.hesa_codes.map { |v| "'#{v}'" }.sort.join(', ')}.",
              )
            end
          end
          ...
          context "when AcademicCycle::for_date is 2021" do
            ...
            it do
              expect(subject).to validate_inclusion_of(:training_route)
                .in_array(ReferenceData::TRAINING_ROUTES.names_with_hesa_codes.excluding(ReferenceData::TRAINING_ROUTES.provider_led_postgrad.name))
                .with_message(/has invalid reference data value of '.*'/)
            end
          end
      ...
      context "when training_route is present" do
        context "when requires_degree? is true" do
          ...
          context "with present degrees_attributes" do
            before do
              subject.degrees_attributes = [Api::V20250::DegreeAttributes.new({})]
            end
  ...
  describe "nested attribute validations" do
    let(:placement_attributes) { [Api::V20250::PlacementAttributes.new({})] }
    let(:degrees_attributes) { [Api::V20250::DegreeAttributes.new({})] }
    let(:nationalisations_attributes) { [Api::V20250::NationalityAttributes.new({})] }
    let(:hesa_trainee_detail_attributes) { Api::V20250::HesaTraineeDetailAttributes.new({}) }
  ...
  describe ".from_trainee" do
    ...
    context "with empty params" do
      ...
      it "pulls HesaTraineeDetail attributes from association" do
        ...
        Api::V20250::HesaTraineeDetailAttributes::ATTRIBUTES.each do |attr|
      ...
      context "changing a single hesa trainee detail attribute" do
        ...
        let(:hesa_trainee_detail_attributes) {
          hesa_trainee_detail.attributes.select { |k, _v|
            Api::V20250::HesaTraineeDetailAttributes::ATTRIBUTES.include?(k.to_sym)
          }
        }
