...
RSpec.describe "POST /api/v1.0/trainees/:trainee_id/recommend-for-qts", feature_register_api: true do
  ...
  describe "success" do
    ...
    it "recommends the trainee for a qts award" do
      post "/api/v1.0/trainees/#{trainee.slug}/recommend-for-qts",
  ...
  describe "failure" do
    context "when the trainee cannot be found" do
      ...
      it "returns 404" do
        post "/api/v1.0/trainees/#{other_trainee.slug}/recommend-for-qts",
             headers: { authorization: "Bearer #{token}" },
             params: { data: { qts_standards_met_date: Time.zone.today } }, as: :json
