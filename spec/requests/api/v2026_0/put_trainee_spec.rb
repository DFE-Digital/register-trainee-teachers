...
describe "`PUT /api/v2026.0/trainees/:id` endpoint" do
  ...
  context "with a valid authentication token" do
    ...
    let(:endpoint) { "/api/v2026.0/trainees/#{slug}" }
    ...
    describe "with training_route" do
      ...
      context "when not present" do
        ...
        it do
          ...
          expect(response.parsed_body[:data][:training_route]).to eq(
            ReferenceData::TRAINING_ROUTES.provider_led_postgrad.hesa_code,
          )
      ...
      context "when invalid" do
        ...
        it do
          expect(response).to have_http_status(:unprocessable_entity)
          expect(response.parsed_body[:errors]).to contain_exactly(
            "training_route has invalid reference data value of 'provider_led_postgrad'. Valid values are '02', '03', '09', '10', '11', '14'.",
          )
        end
        ...
        context "with enhanced errors" do
          ...
          it do
            ...
            expect(response.parsed_body[:errors]).to eq(
              "training_route" => [
                "has invalid reference data value of 'provider_led_postgrad'. Valid values are '02', '03', '09', '10', '11', '14'.",
              ],
            )
          end
        end
  ...
  context "Updating a newly created trainee" do
    ...
    let(:params_for_create) do
      {
        data: {
          ...
          nationality: "GB",
          training_route: ReferenceData::TRAINING_ROUTES.provider_led_undergrad.hesa_code,
          ...
          study_mode: ReferenceData::TRAINEE_STUDY_MODES.full_time.hesa_codes.last,
          degrees_attributes: [
            {
              grade: "02",
              subject: "100425",
    ...
    context "when the trainee doesn't have an existing hesa_trainee_detail record" do
      ...
      before do
        ...
        post "/api/v2026.0/trainees", params: params_for_create.to_json, headers: headers
      ...
      context "when creating a new trainee with #{cs} course with valid params" do
        ...
        context "with a fund_code is ineligible for funding" do
          let(:params_for_update) do
            {
              data: {
                training_route: ReferenceData::TRAINING_ROUTES.opt_in_undergrad.hesa_code,
          ...
          before do
            put("/api/v2026.0/trainees/#{slug}", params: params_for_update.to_json, headers: headers)
          end
