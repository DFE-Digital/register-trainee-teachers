...
describe "`POST /api/v2026.0/trainees` endpoint" do
      ...
      context "when lead_partner_urn is present" do
        ...
        context "when employing_school_urn is present" do
          ...
          context "when applicable school urn is not valid" do
            let(:training_route) { ReferenceData::TRAINING_ROUTES.teacher_degree_apprenticeship.hesa_code }
            let(:params) do
              {
                data: data.merge(
                  lead_partner_urn: "900020",
            ...
            let(:params) do
              {
                data: data.merge(
                  lead_partner_urn: lead_partner.urn,
                  employing_school_urn: employing_school.urn,
                  training_route: ReferenceData::TRAINING_ROUTES.teacher_degree_apprenticeship.hesa_code,
                ),
              }
            end
        ...
        let(:params) do
          {
            data: data.merge(
              ...
              training_route: ReferenceData::TRAINING_ROUTES.provider_led_postgrad.hesa_code,
            ),
          }
        end
  ...
  context "with a fund_code that is ineligible for funding" do
    before do
      params[:data][:training_route] = ReferenceData::TRAINING_ROUTES.teacher_degree_apprenticeship.hesa_code
  ...
  context "with a fund_code that is eligible for funding because it has a special case course subject" do
    ...
    before do
      params[:data][:training_route] = ReferenceData::TRAINING_ROUTES.provider_led_postgrad.hesa_code
      params[:data][:fund_code] = Hesa::CodeSets::FundCodes::NOT_ELIGIBLE
      params[:data][:funding_method] = Hesa::CodeSets::BursaryLevels::POSTGRADUATE_BURSARY
