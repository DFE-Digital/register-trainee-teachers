  ...
  context "with a valid authentication token" do
    ...
    context "non existant trainee" do
      ...
      it "returns status code 404 with a valid JSON response" do
        post(
          "/api/v2027.0/trainees/#{trainee_id}/withdraw",
    ...
    context "with a withdrawable trainee" do
      ...
      it "uses the trainee serializer", openapi: false do
        expect(Api::V20270::TraineeSerializer).to receive(:new).with(trainee).and_return(double(as_hash: trainee.attributes)).at_least(:once)
      ...
      context "with invalid params" do
        context "with empty withdrawal_date" do
          ...
          it "did not change the trainee" do
            expect {
              post "/api/v2027.0/trainees/#{trainee_id}/withdraw", headers: { Authorization: "Bearer bat", **json_headers }
