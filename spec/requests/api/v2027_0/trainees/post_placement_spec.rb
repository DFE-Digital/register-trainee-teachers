...
describe "`POST /trainees/:trainee_slug/placements/` endpoint" do
  context "with a valid authentication token" do
    ...
    let(:placement_attribute_keys) { Api::V20270::PlacementAttributes::ATTRIBUTES }
    ...
    context "when the trainee is in an awarded state" do
      ...
      before do
        post(
          "/api/v2027.0/trainees/#{trainee_slug}/placements",
    ...
    context "with a valid trainee and placement" do
      context "create placement with a school" do
        ...
        it "creates a new placement and returns a 201 (created) status" do
          expect {
            post "/api/v2027.0/trainees/#{trainee_slug}/placements", params: params.to_json, headers: { Authorization: token, **json_headers }
          }.to change {
        ...
        context "with different trainee" do
          ...
          let(:params) do
            { data: attributes_for(:placement).slice(*placement_attribute_keys) }.with_indifferent_access
          end
