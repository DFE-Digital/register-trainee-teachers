...
describe "`PUT /trainees/:trainee_slug/placements/:slug` endpoint" do
  context "with a valid authentication token" do
    ...
    let(:placement_attribute_keys) { Api::V20270::PlacementAttributes::ATTRIBUTES.map(&:to_s) }
    ...
    context "when the trainee is in an awarded state" do
      ...
      before do
        put(
          "/api/v2027.0/trainees/#{trainee_slug}/placements/#{slug}",
    ...
    context "with a valid trainee and placement" do
      describe "update placement with a school" do
        ...
        context "when the params include the school urn" do
          ...
          it "updates an existing placement and returns a 200 (ok) status" do
            expect {
              put "/api/v2027.0/trainees/#{trainee_slug}/placements/#{slug}", params: params.to_json, headers: { Authorization: token, **json_headers }
            }.not_to change {
      ...
      describe "updates an existing placement without a school" do
        ...
        it "partial update of an existing placement returns status code 200 (ok) status" do
          expect {
            put "/api/v2027.0/trainees/#{trainee_slug}/placements/#{slug}", params: params_to_update_postcode.to_json, headers: { Authorization: token, **json_headers }
          }.not_to change {
            trainee.placements.count
          }
    ...
    context "with an invalid placement attributes" do
      let!(:placement) { create(:placement, trainee:) }
      let(:params) do
        { data: Api::V20270::PlacementAttributes::ATTRIBUTES.index_with { |_| nil } }
      end
