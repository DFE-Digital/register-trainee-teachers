  ...
  context "with a valid authentication token" do
    ...
    context "with a valid trainee that has a degree" do
      it "returns status code 200 with a valid JSON response" do
        get "/api/v2027.0//trainees/#{trainee_slug}/degrees/#{slug}", headers: { Authorization: token }
    ...
    context "with a subject label that shares a HESA code" do
      ...
      before { get "/api/v2027.0//trainees/#{trainee_slug}/degrees/#{slug}", headers: { Authorization: token } }
