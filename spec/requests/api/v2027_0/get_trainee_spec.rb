...
describe "`GET /api/v2027.0/trainees/:id` endpoint" do
  ...
  it_behaves_like "a register API endpoint", "/api/v2027.0/trainees/12345"
  ...
  context "when the trainee exists" do
    ...
    it "returns the trainee" do
      parsed_trainee = JSON.parse(Api::GetVersionedItem.for_serializer(model: :trainee, version: "v2027.0").new(trainee).as_hash.to_json)
  ...
  context "when a nationality shares a HESA code" do
    ...
    before { get("/api/v2027.0/trainees/#{trainee.slug}", headers: { Authorization: "Bearer #{token}" }) }
  ...
  context "when the trainee does not exist" do
    before do
      get(
        "/api/v2027.0/trainees/nonexistent",
        headers: { Authorization: "Bearer #{token}" },
      )
    end
