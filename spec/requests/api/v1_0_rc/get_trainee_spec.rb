...
describe "`GET /api/v1.0-rc/trainees/:id` endpoint" do
  ...
  it_behaves_like "a register API endpoint", "/api/v1.0-rc/trainees/12345", "trainee_token"
  ...
  context "when the trainee does not exist" do
    before do
      get(
        "/api/v1.0-rc/trainees/nonexistent",
        headers: { Authorization: "Bearer #{token}" },
      )
    end
