...
describe "`PUT /api/v2026.1/trainees/:id` endpoint" do
  ...
  context "with a valid authentication token" do
    ...
    let(:endpoint) { "/api/v2026.1/trainees/#{slug}" }
    ...
    describe "with training_route" do
      ...
      context "when invalid" do
        ...
        it do
          expect(response).to have_http_status(:unprocessable_entity)
          expect(response.parsed_body[:errors]).to contain_exactly(
            "training_route has invalid reference data value of 'provider_led_postgrad'. Valid values are '03', '09', '10', '11', '12', '14', '15', '16', '17'.",
          )
        end
        ...
        context "with enhanced errors" do
          ...
          it do
            ...
            expect(response.parsed_body[:errors]).to eq(
              "training_route" => [
                "has invalid reference data value of 'provider_led_postgrad'. Valid values are '03', '09', '10', '11', '12', '14', '15', '16', '17'.",
              ],
            )
          end
        end
  ...
  context "Updating a newly created trainee" do
    ...
    context "when the trainee doesn't have an existing hesa_trainee_detail record" do
      ...
      before do
        ...
        post "/api/v2026.1/trainees", params: params_for_create.to_json, headers: headers
    ...
    [CourseSubjects::PHYSICS, CourseSubjects::BIOLOGY].each do |cs|
      context "when creating a new trainee with #{cs} course with valid params" do
        ...
        context "with a fund_code is ineligible for funding" do
          ...
          before do
            put("/api/v2026.1/trainees/#{slug}", params: params_for_update.to_json, headers: headers)
          end
