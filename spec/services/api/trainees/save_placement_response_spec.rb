...
describe Api::Trainees::SavePlacementResponse do
  ...
  context "with a new placement" do
    ...
    context "with valid params" do
      let(:placement_attribute_keys) { Api::V20261::PlacementAttributes::ATTRIBUTES.map(&:to_s).push("address") }

      let(:attributes) do
        attrs = create(:placement).attributes.slice(*placement_attribute_keys).with_indifferent_access
        Api::V20261::PlacementAttributes.new(attrs)
      end
      ...
      it "uses the serializer" do
        expect(Api::V20261::PlacementSerializer).to receive(:new).with(placement).and_return(double(as_hash: placement.attributes)).at_least(:once)
    ...
    context "with invalid params" do
      let(:attributes) { Api::V20261::PlacementAttributes.new({}) }
