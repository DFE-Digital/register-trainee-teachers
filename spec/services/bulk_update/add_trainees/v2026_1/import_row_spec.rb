...
module BulkUpdate
  module AddTrainees
    RSpec.describe V20261::ImportRow do
      before do
      ...
      let(:csv) { Rails.root.join("spec/fixtures/files/bulk_update/trainee_uploads/v2026_1/five_trainees.csv").read }
      ...
      describe "#call" do
        ...
        context "when the row is invalid" do
          let(:csv) { Rails.root.join("spec/fixtures/files/bulk_update/trainee_uploads/v2026_1/five_trainees_with_failed.csv").read }
        ...
        context "when the row is invalid with customised field names" do
          let(:csv) { Rails.root.join("spec/fixtures/files/bulk_update/trainee_uploads/v2026_1/five_trainees_with_three_errors.csv").read }
          ...
          it "does not create a trainee record" do
            ...
            expect(result.errors).to include("degree_graduation_year Enter a valid graduation year")
            expect(result.errors).to include("degree_graduation_year Enter a year that is in the past, for example 2014")
            expect(result.errors).to include("degree_grade must be entered if specifying a previous UK degree or non-UK degree")
            expect(result.errors).to include("itt_start_date can't be blank")
            expect(result.errors).to include("date_of_birth can't be blank")
            expect(result.errors).to include(/itt_aim has invalid reference data value of '.*'/)
            expect(result.errors).to include(/itt_qualification_aim has invalid reference data value of '.*'/)
          end
        end
        ...
        context "when the row is valid and includes placement data" do
          let(:csv) { Rails.root.join("spec/fixtures/files/bulk_update/trainee_uploads/v2026_1/five_trainees_with_placement.csv").read }
        ...
        context "when the row is valid and includes degree data" do
          let(:csv) { Rails.root.join("spec/fixtures/files/bulk_update/trainee_uploads/v2026_1/five_trainees_with_degree.csv").read }
          let(:row) { parsed_csv.first }
        ...
        context "when the row is valid and includes disabilities data" do
          let(:csv) { Rails.root.join("spec/fixtures/files/bulk_update/trainee_uploads/v2026_1/five_trainees_with_disability.csv").read }
          let(:row) { parsed_csv[0] }
