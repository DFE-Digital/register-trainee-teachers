...
RSpec.describe BulkUpdate::AddTrainees::V20261::ImportRows do
  ...
  describe "#call" do
    ...
    context "when the feature flag is off", feature_bulk_add_trainees: false do
      ...
      it "does not call the `ImportRow` service" do
        allow(BulkUpdate::AddTrainees::V20261::ImportRow).to receive(:call)

        described_class.call(trainee_upload)

        expect(BulkUpdate::AddTrainees::V20261::ImportRow).not_to have_received(:call)
      end
    end
    ...
    context "when the feature flag is on", feature_bulk_add_trainees: true do
      ...
      context "when all rows are valid and can be imported" do
        context "when the upload status is pending" do
          ...
          it "does not create any trainee records" do
            allow(BulkUpdate::AddTrainees::V20261::ImportRow).to receive(:call).and_call_original

            expect(described_class.call(trainee_upload)).to be(true)
            expect(BulkUpdate::AddTrainees::V20261::ImportRow).to have_received(:call).exactly(5).times
          end
        ...
        context "when the upload status is in progress" do
          ...
          context "when there is a database error" do
            before do
              allow(Trainees::SubmitForTrn).to receive(:call).and_call_original
              allow(BulkUpdate::AddTrainees::V20261::ImportRow).to receive(:call).and_raise(ActiveRecord::ActiveRecordError)
            end
      ...
      context "when some rows are valid and can be imported and some rows are blank" do
        context "when the upload status is pending" do
          ...
          it "does not create any trainee records" do
            ...
            expect(BulkUpdate::AddTrainees::V20261::ImportRow).to have_received(:call).exactly(3).times
          end
      ...
      context "when some rows are valid and can be imported whilst others are not" do
        ...
        before do
          ...
          allow(BulkUpdate::AddTrainees::V20261::ImportRow).to receive(:call).and_return(
            BulkUpdate::AddTrainees::V20261::ImportRow::Result.new({}, true, []),
            BulkUpdate::AddTrainees::V20261::ImportRow::Result.new({}, true, []),
            BulkUpdate::AddTrainees::V20261::ImportRow::Result.new(
              nil,
              false,
      ...
      context "when some rows cause an exception in the API service classes" do
        ...
        before do
          allow(BulkUpdate::AddTrainees::V20261::ImportRow).to receive(:call).and_raise(StandardError.new("Server on fire"))
        end
  ...
  describe "documentation cross-checks" do
    let(:documentation_fields) { YAML.load_file(BulkUpdate::AddTrainees::V20261::ImportRows.fields_definition_path) }

    BulkUpdate::AddTrainees::V20261::ImportRows::TRAINEE_HEADERS.each_value do |id|
      it "documents field with id #{id}" do
        expect(documentation_fields.map { |field| field["technical"] }).to include(id)
      end
    end

    BulkUpdate::AddTrainees::V20261::ImportRows::TRAINEE_HEADERS.each_key do |name|
  ...
  describe "template CSV cross-checks" do
    let(:file_path) { Rails.public_path.join("csv/v2026_1/bulk_create_trainee.csv") }
    let(:headers) { CSVSafe.new(File.open(file_path), headers: true, encoding: "UTF-8").read.headers }

    BulkUpdate::AddTrainees::V20261::ImportRows::ALL_HEADERS.each_key do |name|
