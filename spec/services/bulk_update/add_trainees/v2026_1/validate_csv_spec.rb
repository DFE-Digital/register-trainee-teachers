  ...
  module AddTrainees
    describe ValidateCsv do
      ...
      context "given a CSV with missing columns" do
        let(:file_path) { Rails.root.join("spec/fixtures/files/bulk_update/trainee_uploads/v2026_1/five_trainees_with_missing_column.csv") }
      ...
      context "given a CSV with the correct columns in the 'wrong' order" do
        let(:file_content) do
          "#{BulkUpdate::AddTrainees::V20261::ImportRows::ALL_HEADERS.keys.reverse.join(',')}\nfoo,bar,baz"
        end
      ...
      context "given a CSV with the correct columns except one that is an empty string" do
        let(:file_content) do
          "#{BulkUpdate::AddTrainees::V20261::ImportRows::ALL_HEADERS.keys.join(',').gsub('first_names', '')}\nfoo,bar,baz"
        end
        let(:csv) { CSVSafe.new(file_content, headers: true).read }
      ...
      context "given a CSV with the correct columns and one extra column" do
        let(:file_content) do
          "#{(BulkUpdate::AddTrainees::V20261::ImportRows::ALL_HEADERS.keys + ['Shoe Size']).join(',')}\nfoo,bar,baz"
        end
        let(:csv) { CSVSafe.new(file_content, headers: true).read }
      ...
      context "given a CSV with the correct columns with validation results and errors" do
        let(:file_content) do
          "#{BulkUpdate::AddTrainees::V20261::ImportRows::ALL_HEADERS.keys.reverse.join(',')},Validation results,Errors\nfoo,bar,baz"
        end
        let(:csv) { CSVSafe.new(file_content, headers: true).read }
      ...
      context "given a CSV file with no data (just a header)" do
        let(:csv) do
          CSVSafe.new(
            BulkUpdate::AddTrainees::V20261::ImportRows::ALL_HEADERS.keys.join(","),
            headers: true,
          ).read
        end
