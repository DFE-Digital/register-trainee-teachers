  ...
  module RecommendationsUploads
    describe ValidateCsv do
      ...
      context "given a CSV with no date header" do
        ...
        it { expect(record.errors.first.message).to eql "The selected file must contain a column for date QTS or EYTS requirements met" }
      end

      context "given a CSV with no dates" do
        let(:csv) { create_recommendations_upload_csv! }

        it { expect(record.errors.first.message).to eql "The selected file must contain date QTS or EYTS requirements met for at least one trainee" }
      end
