  ...
  module Placements
    RSpec.describe ImportRow do
      describe "#call" do
        ...
        let(:service) { described_class.call(placement_row, trainee) }
        let(:trainee) { create(:trainee, :trn_received, :without_placements) }
        let(:dupe_trainee) { create(:trainee, :trn_received, :without_placements, trn: trainee.trn) }
        ...
        context "when the row can be imported and is valid" do
          ...
          it "updates the correct trainee placement" do
            ...
            expect(dupe_trainee.placements.count).to eq(0)
          end
        end
