    ...
    describe "#call" do
      ...
      context "when there is one course with a missing provider" do
        it "reports the number of missing courses" do
          ...
          expect(subject[:message]).to eq("There is 1 course with a missing provider for recruitment cycle year #{recruitment_cycle_year}. The missing provider code is A002.")
        end
      end
      ...
      context "when there are multiple courses with a missing provider" do
        it "reports the number of missing courses" do
          ...
          expect(subject[:message]).to eq("There are 2 courses with missing providers for recruitment cycle year #{recruitment_cycle_year}. The missing provider codes are A002, A003.")
        end
      end
      ...
      context "when there are no courses with missing providers" do
        it "reports there are no missing courses" do
          ...
          expect(subject[:message]).to eq("There are no courses with missing providers for recruitment cycle year #{recruitment_cycle_year}.")
        end
      end
    end
  end
