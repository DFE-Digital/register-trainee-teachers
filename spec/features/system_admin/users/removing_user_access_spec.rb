...
feature "Removing user access" do
  ...
  scenario "training partner" do
    ...
    and_i_choose_a_user_to_remove_access_under_lead_partners
...
private
  ...
  def and_the_lead_partner_access_table_is_shown
    expect(admin_user_show_page).to have_content(user.lead_partners.first.name)
  end
  ...
  def then_the_lead_partner_is_no_longer_listed_on_the_user_page
    expect(admin_user_show_page).not_to have_content(user.lead_partners.first.name)
    expect(admin_user_show_page.flash_message).to be_visible
  end
end
