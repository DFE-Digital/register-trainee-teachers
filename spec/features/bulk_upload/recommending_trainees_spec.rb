...
private
  ...
  def given_two_trainees_exist_to_recommend
    @trainees = [
      create(:trainee, :trn_received, :with_employing_school, trn: "2413295", itt_start_date: 1.year.ago, trainee_start_date: 1.year.ago, itt_end_date: Time.zone.today, provider: current_user.organisation),
      create(:trainee, :trn_received, :with_employing_school, trn: "4814731", itt_start_date: 1.year.ago, trainee_start_date: 1.year.ago, itt_end_date: Time.zone.today + 1.month, provider: current_user.organisation),
    ]
  end

  def and_a_discarded_duplicate_trainee_exists
    @duplicate_trainee = create(:trainee, :trn_received, :with_employing_school, trn: "2413295", discarded_at: 1.day.ago, itt_start_date: 1.year.ago, trainee_start_date: 1.year.ago, itt_end_date: Time.zone.today, provider: current_user.organisation)
  end
  ...
  def then_i_see_the_option_to_download_the_empty_template
    expect(recommendations_upload_page).to have_link("Download an empty template")
  end
  ...
  def then_i_see_count_missing_dates
    expect(recommendations_upload_show_page).to have_text("1 trainee who will gain")
    expect(recommendations_upload_show_page).to have_text("1 trainee who will not change status")
  end
  ...
  def when_i_submit_form_with_no_file_attached
    recommendations_upload_fix_errors_page.upload_button.click
  end

  def and_i_submit_form_with_no_file
    edit_recommendations_upload_page.upload_button.click
  end
