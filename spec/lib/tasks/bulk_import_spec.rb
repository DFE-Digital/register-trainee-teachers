...
describe "bulk_import:import" do
  ...
  context "with a preexisting trainee" do
    let(:csv_path) {
      File.join(__dir__, "..", "..", "support", "fixtures", "bulk_import_import.csv")
    }

    it "edits the existing trainee" do
      trainee = create(:trainee, :with_bulk_import_provider, provider: provider, trainee_id: "L0V3LYiD")
  ...
  def expect_trainee_to_have_attributes_from_csv(trainee)
    ...
    expect(trainee.training_route).to eq "bulk_import_postgrad"
