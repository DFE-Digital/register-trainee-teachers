...
shared_examples "rendering course confirmation" do
  ...
  context "default behaviour" do
    ...
    it "renders the course details" do
      expect(rendered_content).to have_text("#{course.name} (#{course.code})")
    end

    it "renders the level" do
      expect(rendered_content).to have_text(course.level.capitalize)
    end

    it "renders the age range" do
      expect(rendered_content).to have_text(age_range_for_summary_view(course.age_range))
    end

    it "renders the start date" do
      expect(rendered_content).to have_text(date_for_summary_view(course.start_date))
    end

    it "renders the duration" do
      expect(rendered_content).to have_text("#{course.duration_in_years} years")
    end
    ...
    if described_class == PublishCourseDetails::View
      ...
      it "renders 8 rows on the confirmation page" do
        expect(rendered_content).to have_css(".govuk-summary-list__row", count: 8)
      end
    end
    ...
    context "non study mode training route" do
      ...
      it "does not render study_mode" do
        expect(rendered_content).not_to have_css(".govuk-summary-list__row.full-time-or-part-time .govuk-summary-list__key", text: "Full time or part time")
      end

      it "does not render the selected study_mode" do
        expect(rendered_content).not_to have_css(".govuk-summary-list__row.full-time-or-part-time .govuk-summary-list__value", text: "Full time")
      end
  ...
  context "course subjects" do
    ...
    context "with one subject" do
      ...
      it "renders the first subject's name" do
        expect(rendered_content).to have_text(expected_names)
      end

      it "displays the correct subject summary label" do
        expect(rendered_content).to have_text(I18n.t("publish_course_details.view.subject"))
      end
    end
    ...
    context "with two subjects" do
      ...
      it "displays the correct subject summary label" do
        expect(rendered_content).to have_text(I18n.t("publish_course_details.view.multiple_subjects"))
      end
    end
