...
# Copy of Api::V20250::HesaMapper::DegreeAttributes (removed in ff0e76ac4), kept so specs can
# check the YAML mapper still matches the gem. Do not update it to match v2026.1.
class GemDegreeAttributesMapper
...
private
  ...
  def uk_degree
    return InvalidValue.new(@params[:uk_degree]) if @params[:uk_degree].present? && uk_degree_type.blank?
  ...
  def non_uk_degree
    return InvalidValue.new(@params[:non_uk_degree]) if @params[:non_uk_degree].present? && non_uk_degree_type.blank?
  ...
  def country_from_mapping
    @country_from_mapping ||= begin
      ...
      if @params[:country].present? && mapped_value.nil?
        InvalidValue.new(@params[:country])
      else
        mapped_value
      end
    end
