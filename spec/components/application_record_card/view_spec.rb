...
module ApplicationRecordCard
  describe View do
    ...
    it "does not render provider name" do
      expect(page).not_to have_selector(".application-record-card__provider_name")
    end
    ...
    context "when system admin user" do
      ...
      it "renders provider name" do
        expect(page).to have_selector(".application-record-card__provider_name", text: provider.name.to_s)
      end

      it "renders the record source" do
        expect(page).to have_selector(".application-record-card__record_source", text: I18n.t("components.application_record_card.record_source.title"))
      end
    end
    ...
    context "when the Trainee has no names" do
      it "renders 'Draft record'" do
        expect(page).to have_text("Draft record")
      end
    end
    ...
    context "when the Trainee has no subject" do
      ...
      context "and is an Early Years trainee" do
        ...
        it "renders 'Early years teaching'" do
          expect(page).to have_text("Early years teaching")
        end
      end
    end
    ...
    context "when the Trainee has no route" do
      ...
      it "renders 'No route provided'" do
        expect(page).to have_text("No route provided")
      end
    end

    context "when the Trainee has no trainee_id" do
      it "does not render trainee ID" do
        expect(page).not_to have_selector(".application-record-card__id")
      end
    end

    context "when the Trainee has no trn" do
      it "does not render trn" do
        expect(page).not_to have_selector(".application-record-card__trn")
      end
    end
      ...
      [
        ...
        context "when state is #{state_expectation[:state]}" do
          ...
          it "renders '#{state_expectation[:text]}'" do
            expect(page).to have_selector(".govuk-tag", text: state_expectation[:text])
          end
          ...
          it "sets the colour to #{state_expectation[:colour]}" do
            ...
            expect(page).to have_selector(expected_tag_class)
          end
        end
      end
    end
    ...
    context "when a trainee with all their details filled in" do
      ...
      it "renders trainee ID" do
        expect(page).to have_text("Trainee ID: 132456")
      end

      it "renders trn" do
        expect(page).to have_text("TRN: 789456")
      end

      it "renders start_year" do
        expect(page).to have_text("Start year: 2019 to 2020")
      end

      it "renders end_year" do
        expect(page).to have_text("End year: 2019 to 2020")
      end

      it "renders trainee name" do
        expect(page).to have_text("Teddy Smith")
      end
      ...
      it "renders route" do
        expect(page).to have_text(t("activerecord.attributes.trainee.training_routes.assessment_only"))
      end

      describe "rendering updated at" do
        context "when the trainee is in draft" do
          it "renders updated at from the model" do
            expect(page).to have_text("Updated: #{trainee.created_at.strftime('%-d %B %Y')}")
          end
        end
        ...
        context "when the trainee is not draft" do
          ...
          it "renders updated at from the timeline" do
            expect(page).to have_text("Updated: #{Time.zone.now.strftime('%-d %B %Y')}")
          end
        end
      end
    end
