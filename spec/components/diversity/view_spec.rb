...
describe Diversity::View do
  ...
  describe "Diversity information" do
    context "when trainee has not shared any ethnic or disability information" do
      ...
      it "renders with one line to say the trainee haven't shared data" do
        expect(rendered_content).to have_text("Not shared")
        expect(rendered_content).to have_css(".govuk-summary-list__row", count: 1)
      end

      it "has one change links" do
        expect(rendered_content).to have_link("Change diversity information")
        expect(rendered_content).not_to have_link("Change ethnicity")
        expect(rendered_content).not_to have_link("Change disability")
      end
    end
    ...
    context "when trainee has shared their diversity information" do
      ...
      it "renders with one line to say the trainee has shared data" do
        expect(rendered_content).to have_text("Information disclosed")
        expect(rendered_content).to have_css(".govuk-summary-list__row", count: 3)
      end

      it "has three change links" do
        expect(rendered_content).to have_link("Change diversity information")
        expect(rendered_content).to have_link("Change ethnicity")
        expect(rendered_content).to have_link("Change disability")
      end
    end
  end
  ...
  describe "ethnic selection" do
    ...
    it "returns ethnicity is missing" do
      expect(rendered_content).to have_text("Ethnicity is missing")
      expect(rendered_content).not_to have_text(locale_key)
    end
    ...
    context "when ethnic_background is not provided" do
      ...
      it "returns the ethnic_group" do
        expect(rendered_content).to have_text(locale_key)
      end
    end
    ...
    context "ethnic background" do
      ...
      context "when ethnic_background provided" do
        it "returns the ethnic_background alongside the ethnic_group" do
          expect(rendered_content).to have_text("#{locale_key} (#{ethnic_background})")
        end
      end
      ...
      context "when additional_ethnic_background provided" do
        ...
        it "returns the additional_ethnic_background alongside the ethnic_group" do
          expect(rendered_content).to have_text("#{locale_key} (#{additional_background})")
        end
      end
    end
  end
  ...
  describe "#disability_selection" do
    ...
    it "returns disability is missing" do
      expect(rendered_content).to have_text("Disability is missing")
    end
    ...
    context "not disabled" do
      ...
      it "returns a message stating the user is not disabled" do
        expect(rendered_content).to have_text("They shared that they’re not disabled")
      end
    end
