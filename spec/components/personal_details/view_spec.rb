  ...
  describe View do
    ...
    context "when data has not been provided" do
      ...
      it "renders blank rows for full name, date of birth, sex and nationality" do
        expect(rendered_content).to have_css(".govuk-summary-list__row", count: 4)
      end
    ...
    context "when data has been provided" do
      ...
      it "renders the full name" do
        ...
        expect(rendered_content).to have_text(expected_name)
      end

      it "renders the date of birth" do
        expected_dob = trainee.date_of_birth.strftime("%-d %B %Y")

        expect(rendered_content).to have_text(expected_dob)
      end
      ...
      it "renders the nationality" do
        expect(rendered_content).to have_text("British")
      end
      ...
      context "when multiple nationalities have been provided" do
        ...
        it "renders a list of nationalities" do
          expect(rendered_content).to have_css(".govuk-summary-list__row.nationality .govuk-list li", count: 2)
        end

        it "renders in the order of first to last" do
          expect(rendered_content).to have_css(".govuk-summary-list__row.nationality .govuk-list li:first-child", text: "British")
          expect(rendered_content).to have_css(".govuk-summary-list__row.nationality .govuk-list li:last-child", text: "Irish")
        end
      end
    end
  end
