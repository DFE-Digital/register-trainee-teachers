    ...
    module ClassMethods
      def hesa_code_inclusion_message(value:, valid_values:)
        if valid_values.count < ApiInclusionValidator::MAX_VALID_VALUES_DISPLAYED
          I18n.t(
            "activemodel.errors.models.api/v20261/trainee_attributes.attributes.inclusion",
            value: value,
            valid_values: valid_values.map { |v| "'#{v}'" }.join(", "),
          )
        else
          I18n.t(
            "activemodel.errors.models.api/v20261/trainee_attributes.attributes.inclusion_with_truncated_list",
            value: value,
