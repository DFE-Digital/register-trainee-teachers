...
module Funding
  class ProviderPaymentSchedulesImporter
    ...
    def call
      attributes.keys.each do |accreditation_id|
        provider = Provider.find_by(accreditation_id: accreditation_id)
        schedule =  provider.funding_payment_schedules.create

        row_attributes = attributes[accreditation_id]
