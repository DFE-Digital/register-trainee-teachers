...
module Hpitt
  class ImportTrainee
    ...
    def call
      ...
      degree = build_degree(trainee)

      trainee.save!
      degree.save!
    end

    def build_degree(trainee)
      if csv_row["Country (Non UK) degree"].blank?
        build_uk_degree(trainee)
      else
        build_non_uk_degree(trainee)
      end
    end

    def build_uk_degree(trainee)
      Degree.new(trainee: trainee).tap do |degree|
        degree.locale_code = :uk
        degree.uk_degree = validate_uk_degree(csv_row["Degree type"])
        degree.grade = to_degree_grade(csv_row["Degree grade"])
        degree.graduation_year = csv_row["Graduation year"]
        degree.institution = validate_degree_institution(csv_row["Institution"])
        degree.subject = validate_degree_subject(csv_row["Degree subject"])
      end
    end

    def build_non_uk_degree(trainee)
      Degree.new(trainee: trainee).tap do |degree|
        degree.locale_code = :non_uk
        degree.country = csv_row["Country (Non UK) degree"]
        degree.non_uk_degree = validate_enic_non_uk_degree(csv_row["UK ENIC equivalent (Non UK)"])
        degree.subject = validate_degree_subject(csv_row["Degree subject"])
        degree.graduation_year = Date.new(csv_row["Undergrad degree date obtained (Non UK)"].to_i).year if csv_row["Undergrad degree date obtained (Non UK)"].present?
      end
    end
