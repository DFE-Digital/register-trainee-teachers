...
list-pglogs: composed-variables set-pgserver set-azure-account
	az postgres flexible-server server-logs list --resource-group ${RESOURCE_GROUP_NAME} --server-name ${SERVERNAME} | grep name
...
show-service: get-cluster-credentials
	$(if $(APP_NAME), $(eval export DSUFFIX="-${APP_NAME}"), $(eval export DSUFFIX="-${CONFIG}") )
	$(eval NAMESPACE=$(shell jq -r '.namespace' terraform/aks/workspace-variables/$(CONFIG).tfvars.json))
